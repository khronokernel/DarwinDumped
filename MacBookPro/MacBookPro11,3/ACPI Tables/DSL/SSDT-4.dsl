 ...
 * 
 * Disassembly of /Users/arinabalerina/Downloads/DarwinDumper_v2.9.8/DarwinDumperReports/000_2015-07-16_17-23-26_MacBookPro11,3/DarwinDumper_2.9.8_Apple_X64_Yos_arinabalerina/ACPI Tables/AML/SSDT-4.aml, Thu Jul 16 17:24:29 2015
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00009E2C (40492)
 *     Revision         0x01
 *     Checksum         0x71
 *     OEM ID           "APPLE "
 ...
 */
DefinitionBlock ("/Users/arinabalerina/Downloads/DarwinDumper_v2.9.8/DarwinDumperReports/000_2015-07-16_17-23-26_MacBookPro11,3/DarwinDumper_2.9.8_Apple_X64_Yos_arinabalerina/ACPI Tables/AML/SSDT-4.aml", "SSDT", 1, "APPLE ", "TbtPEG11", 0x00001000)
{
    ...
    Scope (\_SB.PCI0.P0P2)
    {
        ...
        Device (GFX0)
        {
            ...
            OperationRegion (DPR1, SystemIO, 0x07D0, 0x01)
            Field (DPR1, ByteAcc, NoLock, Preserve)
            {
                GRD,    8
            }

            OperationRegion (DPR2, SystemIO, 0x07D4, 0x01)
            ...
            Method (WFRD, 0, NotSerialized)
            {
                Store (0xC8, Local0)
                Store (GWR, Local1)
                While (LAnd (Local0, LEqual (And (Local1, 0x01), 0x01)))
                {
                    Store (GRD, Local2)
                    Store (GWR, Local1)
                    Stall (0x64)
            ...
            Method (WFCD, 0, NotSerialized)
            {
                Store (0xC8, Local0)
                Store (GWR, Local1)
                While (LAnd (Local0, LEqual (And (Local1, 0x01), 0x00)))
                {
                    Store (GWR, Local1)
                    Stall (0x64)
                    Decrement (Local0)
            ...
            {
                Store (And (0xFF, Arg2), BOX0)
                If (LGreaterEqual (Arg1, 0x02))
                {
                    Store (And (0xFF, ShiftRight (Arg2, 0x08)), BOX1)
                }

                If (LEqual (Arg1, 0x04))
                {
                    Store (And (0xFF, ShiftRight (Arg2, 0x10)), BOX2)
                    Store (And (0xFF, ShiftRight (Arg2, 0x18)), BOX3)
                }

                If (WFRD ()) {}
                Store (And (0xFF, Arg0), GWR)
                If (WFCD ()) {}
            }
            ...
            {
                Store (Arg0, BRTL)
                Divide (Multiply (Arg0, 0x03FF), 0x64, Local1, Local0)
    ...
    OperationRegion (T2PM, SystemMemory, 0xE0200548, 0x08)
    ...
    Scope (\_SB.PCI0)
    {
        ...
        Method (RMCR, 0, Serialized)
        {
            If (LNot (OSDW ()))
            {
                ...
                If (LAnd (LAnd (LEqual (\_SB.PCI0.PEG1.UPSB.DSB1.UPS0.AVND, 0xFFFF), LEqual (\_SB.PCI0.PEG1.UPSB.DSB2.UPS0.AVND, 0xFFFF)), LEqual (
                    \_SB.PCI0.PEG1.UPSB.DSB3.UPS0.AVND, 0xFFFF)))
                {
                    RMC1 ()
                    Return (Zero)
                }
        ...
        Method (RMCW, 0, Serialized)
        {
            If (LNot (OSDW ()))
            {
                ...
                {
                    \_SB.PCI0.PEG1.UPSB.DSB0.NHI0._EJ0 (0x01)
                }
                Else
                {
                    If (LNot (TBBT))
